    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <div style={{ fontSize:18, fontWeight:800, color:'#0f2044' }}>{t('term_title')}</div>
            <div style={{ fontSize:12, color:'#64748b' }}>{total.toLocaleString()} {t('term_records')}</div>
          </div>
        </div>
        <button onClick={() => setSlicerOpen(s => !s)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:8, border:'1.5px solid #e2e8f0', background:'#fff', cursor:'pointer', fontSize:13, fontWeight:600, color:'#374151' }}>
          ...
          {t('filters')} {activeFilters > 0 && <span style={{ background:'#d97706', color:'#fff', borderRadius:99, fontSize:10, padding:'1px 6px' }}>{activeFilters}</span>}
        </button>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1.5fr', gap:12 }}>
        {[
          [t('total_terminated'), total,  '#d97706','#fffbeb'],
          [t('this_page'),        filteredDrivers.length, '#1d4ed8','#eff6ff'],
          [t('unique_reasons'),   Object.keys(reasonCount).length, '#7c3aed','#f5f3ff'],
        ].map(([l,v,c,bg]) => (
        ...
        <div style={{ ...S.card, borderTop:'3px solid #dc2626' }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#64748b', textTransform:'uppercase', letterSpacing:.6, marginBottom:10 }}>{t('top_reasons')}</div>
      ...
      <div style={{ ...S.card, padding:0, overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', borderBottom:'1.5px solid #f1f5f9' }}>
          <div style={{ fontSize:14, fontWeight:700, color:'#0f2044' }}>{t('terminated_records')}</div>
          <div style={{ fontSize:12, fontWeight:600, color:'#64748b', background:'#f1f5f9', padding:'3px 10px', borderRadius:99 }}>{filteredDrivers.length.toLocaleString()} {t('records')}</div>
        </div>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#f8fafc' }}>
                {['#',t('rta_id'),t('full_name'),t('nationality'),t('depot'),t('contractor'),t('date_left'),t('reason')].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{ textAlign:'center', padding:40, color:'#94a3b8' }}>{t('no_data')}</td></tr>
